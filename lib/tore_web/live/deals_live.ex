defmodule ToreWeb.DealsLive do
  ...
  @impl true
  def handle_event("blur_store", %{"value" => new_store, "chain" => chain, "old_store" => old_store}, socket) do
    ...
    {:noreply, assign(socket, editing_store: nil, saved_store: store_key, deals: Deals.list_current())}
  end

  @impl true
  def handle_event("save_store", %{"chain" => chain, "old_store" => old_store, "store" => new_store}, socket) do
    new_store = String.trim(new_store)
  ...
  def handle_info({:pdf_result, {:error, _}}, socket) do
    {:noreply, socket |> assign(scraping: false) |> put_flash(:error, gettext("PDF parse failed"))}
  end
    ...
    <.page max_width={:md}>
      <.card padded={false}>
        ...
        <header class="px-6 pt-6 pb-3 flex items-center justify-between">
          <h1 class="font-semibold text-[var(--text)]" style="font-size: var(--t-h1);">{gettext("Erbjudanden")}</h1>
          <div class="flex items-center gap-1">
            <%= if @scraping do %>
              <span class="text-[color:var(--muted)] inline-flex items-center gap-1.5 mr-1" style="font-size: var(--t-meta);">
            ...
            <button
              ...
              class={[
                ...
                @sources_expanded && "bg-[color:var(--hairline)] text-[var(--text)]" || "text-[color:var(--muted)] hover:text-[var(--text)] hover:bg-[color:var(--hairline)]"
              ]}
            >
              <.icon name={if @sources_expanded, do: "hero-chevron-up", else: "hero-chevron-down"} class="size-4" />
            </button>
            <button
              phx-click="open_add"
        ...
        <%= if @sources_expanded do %>
          <div class="border-t border-[color:var(--hairline)]">
            <%= if @store_configs == [] do %>
              <p class="px-6 py-4 text-[color:var(--muted)]" style="font-size: var(--t-meta);">{gettext("Inga återkommande källor konfigurerade.")}</p>
            <% else %>
              <ul class="divide-y divide-[color:var(--hairline)]">
                <li :for={config <- @store_configs} class="px-6 py-3 flex items-center gap-3">
                  ...
                  <button
                    ...
                    class={["shrink-0 h-6 w-11 rounded-full transition-colors", config.scrape_enabled && "bg-[color:var(--accent)]" || "bg-[color:var(--border)]"]}
                    aria-label={gettext("Växla")}
                  >
                    <span class={["block h-5 w-5 rounded-full bg-white shadow transition-transform mx-0.5", config.scrape_enabled && "translate-x-5" || "translate-x-0"]} />
                  </button>
                  <button
        ...
        <%= if @grouped == [] do %>
          <div class="px-6 py-14 border-t border-[color:var(--hairline)] flex flex-col items-center text-center">
            ...
            <p class="text-[color:var(--muted)]" style="font-size: var(--t-body);">{gettext("Inga aktuella erbjudanden")}</p>
            <p class="mt-1 text-[color:var(--subtle)]" style="font-size: var(--t-meta);">{gettext("Lägg till en källa med + ovan.")}</p>
          </div>
        <% else %>
          <div class="border-t border-[color:var(--hairline)]">
            <%= for {chain, stores} <- @grouped do %>
              ...
              <div class="border-b border-[color:var(--hairline)] last:border-b-0">
                ...
                >
                  <.icon name={if chain_collapsed, do: "hero-chevron-right", else: "hero-chevron-down"} class="size-3.5 text-[color:var(--subtle)] shrink-0" />
                  <span class="font-bold uppercase tracking-wide text-[var(--text)] flex-1" style="font-size: var(--t-meta);">{chain}</span>
                  <span class="inline-flex items-center rounded-full px-2 py-0.5 bg-[color:var(--accent)]/10 text-[color:var(--accent)]" style="font-size: 11px; font-weight: 500;">{chain_total}</span>
                </button>
                ...
                <%= unless chain_collapsed do %>
                  <%= for {store, deals} <- stores do %>
                    ...
                    <div class="flex items-center bg-[color:var(--hairline)]/40 border-t border-[color:var(--hairline)] group/store">
                      ...
                      >
                        <.icon name={if store_collapsed, do: "hero-chevron-right", else: "hero-chevron-down"} class="size-3 text-[color:var(--subtle)]" />
                      </button>
                      ...
                      <% else %>
                        ...
                        >
                          ...
                          <%= if @saved_store == store_key do %>
                            <span class="ml-2 text-[color:var(--accent)] animate-pulse" style="font-size: 10px; font-weight: 400;">Sparat</span>
                          <% end %>
                        </button>
                      <% end %>

                      <span class="px-3 shrink-0 text-[color:var(--subtle)]" style="font-size: 11px;">{length(deals)}</span>
                    </div>
                      ...
                      <ul class="divide-y divide-[color:var(--hairline)]">
                        <li :for={deal <- deals} class="px-6 py-3 flex items-start gap-3">
                          <div class="flex-1 min-w-0">
                            ...
                            <p :if={deal.brand || deal.size} class="text-[color:var(--muted)]" style="font-size: var(--t-meta);">
                          ...
                          <div class="shrink-0 text-right">
                            ...
                            <p :if={deal.price} class="font-semibold text-[var(--text)]" style="font-size: var(--t-body);">{deal.price} {deal.price_unit || "kr"}</p>
      ...
      >
        <div class="bg-[var(--surface)] rounded-[var(--r-xl)] shadow-[0_8px_32px_rgba(17,24,39,0.16)] overflow-hidden">
          ...
          <header class="flex items-center justify-between px-6 py-4 border-b border-[color:var(--hairline)]">
            <h2 class="font-semibold text-[var(--text)]" style="font-size: var(--t-h2);">{gettext("Lägg till källa")}</h2>
            <button
              type="button"
              phx-click="close_add"
          ...
          <div class="flex border-b border-[color:var(--hairline)]">
            <button
              ...
              class={[
                ...
                @modal.tab == "url" && "text-[color:var(--accent)] border-b-2 border-[color:var(--accent)] -mb-px font-medium" || "text-[color:var(--muted)] hover:text-[var(--text)]"
              ]}
            ...
            <button
              ...
              class={[
                "flex-1 py-3 text-center transition-colors",
                @modal.tab == "pdf" && "text-[color:var(--accent)] border-b-2 border-[color:var(--accent)] -mb-px font-medium" || "text-[color:var(--muted)] hover:text-[var(--text)]"
              ]}
              style="font-size: var(--t-meta);"
            >
              {gettext("PDF")}
            ...
            <div class="p-6 space-y-4">
              ...
              <div class="flex rounded-[var(--r-md)] border border-[color:var(--border)] overflow-hidden">
                <button
                  ...
                  class={[
                    ...
                    @modal.url_mode == "once" && "bg-[color:var(--accent)] text-white font-medium" || "text-[color:var(--muted)] hover:bg-[color:var(--hairline)]"
                  ]}
                  style="font-size: var(--t-meta);"
                >
                ...
                <button
                  ...
                  class={[
                    "flex-1 py-2 text-center transition-colors",
                    @modal.url_mode == "recurring" && "bg-[color:var(--accent)] text-white font-medium" || "text-[color:var(--muted)] hover:bg-[color:var(--hairline)]"
                  ]}
                  style="font-size: var(--t-meta);"
                >
              ...
              <%= if @modal.url_mode == "once" do %>
                <form phx-submit="scrape_once" class="space-y-3">
                  <.field name="name" label={gettext("Butiksnamn")} placeholder="ICA Kvantum Sickla" required />
                  <.field name="url" label={gettext("URL till erbjudanden")} placeholder="https://www.ica.se/erbjudanden/..." required />
                  <.button type="submit" variant={:primary} size={:md} class="w-full" disabled={@scraping}>
            ...
            <div class="p-6 space-y-3">
              <form phx-change="validate_upload">
                ...
                >
                  ...
                  <p class="text-[color:var(--muted)]" style="font-size: var(--t-body);">{gettext("Släpp en PDF här eller")}</p>
                  <label for={@uploads.deals_pdf.ref} class="cursor-pointer text-[color:var(--accent)] hover:underline" style="font-size: var(--t-body);">
                    {gettext("välj fil")}
                    <.live_file_input upload={@uploads.deals_pdf} class="hidden" />
                  </label>
                  <p class="text-[color:var(--subtle)]" style="font-size: var(--t-meta);">{gettext("Erbjudanden extraheras automatiskt · max 20 MB")}</p>

                  <%= for entry <- @uploads.deals_pdf.entries do %>
                    <div class="w-full mt-3">
                      <div class="flex items-center gap-2 text-[var(--text)]" style="font-size: var(--t-meta);">
                      ...
                      <div class="mt-1 h-1 rounded-full bg-[color:var(--hairline)] overflow-hidden">
                        <div class="h-full bg-[color:var(--accent)] transition-all" style={"width: #{entry.progress}%"} />
                      </div>
                      <%= for err <- upload_errors(@uploads.deals_pdf, entry) do %>
                        <p class="text-[color:var(--danger)] mt-1" style="font-size: var(--t-meta);">{upload_error_to_string(err)}</p>
                      <% end %>
                    </div>
                  <% end %>
                </div>
