defmodule ToreWeb.CookingLive do
  ...
  def handle_event("set_cuisine", %{"cuisine" => cuisine, "rating" => rating}, socket) do
    ...
    updated = if rating == "neutral", do: Map.delete(current, cuisine), else: Map.put(current, cuisine, rating)
    ...
    <.page max_width={:md}>
      <.card padded={false}>
        <header class="px-6 pt-6 pb-4">
          <h1 class="font-semibold text-[var(--text)]" style="font-size: var(--t-h1);">{gettext("Household kitchen")}</h1>
          <p class="text-[color:var(--muted)] mt-1" style="font-size: var(--t-meta);">{gettext("Used when planning dinners, leftovers, and grocery lists.")}</p>
        </header>

        <%!-- We eat --%>
        <section class="grid grid-cols-[1fr_2fr] gap-x-6 px-6 py-5 border-t border-[color:var(--hairline)]">
          <div>
            <p class="font-medium text-[var(--text)]" style="font-size: var(--t-body);">{gettext("We eat")}</p>
            <p class="text-[color:var(--muted)] mt-0.5" style="font-size: var(--t-meta);">{gettext("Tell us what we should cook around.")}</p>
          </div>
        ...
        <section class="grid grid-cols-[1fr_2fr] gap-x-6 px-6 py-5 border-t border-[color:var(--hairline)]">
          <div>
            <p class="font-medium text-[var(--text)]" style="font-size: var(--t-body);">{gettext("Never include")}</p>
            <p class="text-[color:var(--muted)] mt-0.5" style="font-size: var(--t-meta);">{gettext("Allergies and hard avoids — never suggested.")}</p>
          </div>
          <div class="flex flex-wrap gap-2">
            <button
              :for={a <- @allergies}
        ...
        <section class="grid grid-cols-[1fr_2fr] gap-x-6 px-6 py-5 border-t border-[color:var(--hairline)]">
          <div>
            <p class="font-medium text-[var(--text)]" style="font-size: var(--t-body);">{gettext("Our kitchen")}</p>
            <p class="text-[color:var(--muted)] mt-0.5" style="font-size: var(--t-meta);">{gettext("How we like to cook and what matters to us.")}</p>
          </div>
          <div class="flex flex-wrap gap-2">
            <button
              :for={s <- @cooking_style}
        ...
        <section class="grid grid-cols-[1fr_2fr] gap-x-6 px-6 py-5 border-t border-[color:var(--hairline)]">
          <div>
            <p class="font-medium text-[var(--text)]" style="font-size: var(--t-body);">{gettext("Flavours we enjoy")}</p>
            <p class="text-[color:var(--muted)] mt-0.5" style="font-size: var(--t-meta);">{gettext("Cuisines and styles we want more or less of.")}</p>
          </div>
          <div class="flex flex-col gap-2">
            <div :for={cuisine <- @cuisines} class="flex items-center justify-between gap-3">
              <span class="text-[var(--text)]" style="font-size: var(--t-body);">{label_for(cuisine)}</span>
              <div class="flex items-center gap-1 shrink-0">
                <button
                  type="button"
                  phx-click="set_cuisine"
                  phx-value-cuisine={cuisine}
                  phx-value-rating={if Map.get(@prefs.cuisine_preferences || %{}, cuisine) == "less", do: "neutral", else: "less"}
                  class={[
                ...
                <button
                  ...
                  phx-click="set_cuisine"
                  phx-value-cuisine={cuisine}
                  phx-value-rating={if Map.get(@prefs.cuisine_preferences || %{}, cuisine) == "more", do: "neutral", else: "more"}
                  class={[
                    "size-8 rounded-full border flex items-center justify-center transition-colors",
        ...
        <section class="grid grid-cols-[1fr_2fr] gap-x-6 px-6 py-5 border-t border-[color:var(--hairline)]">
          <div>
            <p class="font-medium text-[var(--text)]" style="font-size: var(--t-body);">{gettext("Avoid too often")}</p>
            <p class="text-[color:var(--muted)] mt-0.5" style="font-size: var(--t-meta);">{gettext("Soft dislikes — helps us keep it varied.")}</p>
          </div>
          <div>
        ...
        <section class="grid grid-cols-[1fr_2fr] gap-x-6 px-6 py-5 border-t border-[color:var(--hairline)]">
          <div>
            <p class="font-medium text-[var(--text)]" style="font-size: var(--t-body);">{gettext("Our routine")}</p>
            <p class="text-[color:var(--muted)] mt-0.5" style="font-size: var(--t-meta);">{gettext("Portions and planning schedule.")}</p>
          </div>
          <div class="flex flex-col gap-4">
            <div class="flex items-center justify-between gap-3">
              <span class="text-[var(--text)]" style="font-size: var(--t-body);">{gettext("Dinner portions")}</span>
              <div class="flex items-center gap-2 shrink-0">
                <button type="button" phx-click="dec_portions" phx-value-field="default_portions"
                ...
                <span class="w-6 text-center font-medium tabular-nums text-[var(--text)]" style="font-size: var(--t-body);">{@prefs.default_portions}</span>
                <button type="button" phx-click="inc_portions" phx-value-field="default_portions"
                  class="size-8 rounded-full border border-[color:var(--hairline)] bg-[color:var(--surface)] flex items-center justify-center text-[color:var(--muted)] hover:border-[color:var(--accent)] hover:text-[color:var(--accent)] transition-colors">
                  <.icon name="hero-plus" class="size-3.5" />
                </button>
              </div>
            ...
            <div class="flex items-center justify-between gap-3">
              <span class="text-[var(--text)]" style="font-size: var(--t-body);">{gettext("Leftovers (matlådor)")}</span>
              <div class="flex items-center gap-2 shrink-0">
                <button type="button" phx-click="dec_portions" phx-value-field="default_leftover_portions"
                  class="size-8 rounded-full border border-[color:var(--hairline)] bg-[color:var(--surface)] flex items-center justify-center text-[color:var(--muted)] hover:border-[color:var(--accent)] hover:text-[color:var(--accent)] transition-colors">
                  <.icon name="hero-minus" class="size-3.5" />
                </button>
                <span class="w-6 text-center font-medium tabular-nums text-[var(--text)]" style="font-size: var(--t-body);">{@prefs.default_leftover_portions}</span>
                <button type="button" phx-click="inc_portions" phx-value-field="default_leftover_portions"
                  class="size-8 rounded-full border border-[color:var(--hairline)] bg-[color:var(--surface)] flex items-center justify-center text-[color:var(--muted)] hover:border-[color:var(--accent)] hover:text-[color:var(--accent)] transition-colors">
                  <.icon name="hero-plus" class="size-3.5" />
                </button>
              </div>
            ...
            <div class="flex items-center justify-between gap-3">
              <span class="text-[var(--text)]" style="font-size: var(--t-body);">{gettext("Include lunches")}</span>
              <button
                type="button"
                ...
                class={[
                  ...
                  !@prefs.include_lunches && "bg-[color:var(--hairline)] border-[color:var(--hairline)]"
                ]}
              >
                <span class={[
            ...
            <div class="flex items-center justify-between gap-3">
              <span class="text-[var(--text)]" style="font-size: var(--t-body);">{gettext("Plan days")}</span>
