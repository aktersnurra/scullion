defmodule Scullion.Handlers.CostsHandlerTest do
  ...
  setup do
    ...
    {:ok, {user, _code}} = Scullion.Accounts.create_user(%{name: "Tester"})
    %{user_id: user.id}
  end
  ...
  test "parse_and_log_receipt/2 calls LLM and persists receipt", %{user_id: user_id} do
    ...
    assert {:ok, receipt} = Scullion.Handlers.CostsHandler.parse_and_log_receipt(<<1, 2, 3>>, user_id)
  ...
  test "parse_and_log_receipt/2 returns error when LLM fails", %{user_id: user_id} do
    ...
    assert {:error, :rate_limited} = Scullion.Handlers.CostsHandler.parse_and_log_receipt(<<1>>, user_id)
  end
  ...
  test "log_dining_out/2 delegates to Costs context", %{user_id: user_id} do
    ...
    assert {:ok, entry} = Scullion.Handlers.CostsHandler.log_dining_out(attrs, user_id)
