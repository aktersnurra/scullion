defmodule Tore.CostsTest do
  use Tore.DataCase, async: false

  alias Tore.{Costs, Accounts}
  ...
  test "log_receipt/1 inserts line items", %{user_id: user_id} do
    ...
    assert {:ok, receipt} = Costs.log_receipt(attrs)
    loaded = Tore.Repo.preload(receipt, :line_items)
  ...
  test "cost_per_meal/1 returns error for zero meal count" do
    assert {:error, :invalid_period} =
             Costs.cost_per_meal(%{week_start: Date.utc_today(), meal_count: 0})
  end
end
